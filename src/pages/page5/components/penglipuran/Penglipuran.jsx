import './penglipuran.css';
import { motion } from 'framer-motion';
const Penglipuran = () => {
    return (
        <motion.div 
            initial={{ width: "0px" }}
            animate={{ width: "200vw" }}
            transition={{ duration: 1.5 }}
            className="penglipuranSection">
            <div className='penglipuranSectionKiri'>
                <img src="./images/tengok-ragam-real/Page 3/aset1 pg3.png" alt="" />
                <br />
                <div>
                    <p style={{fontSize:'2vw',textAlign:'center'}}>DESA <b>PENGLIPURAN</b>  merupakan desa tradisional yang terletak di Kabupaten <br />
                    Bangli, Bali. Desa ini terkenal karena keindahan alam, arsitektur rumah tradisional, dan <br />
                    kehidupan masyarakatnya yang masih memegang kuat nilai-nilai budaya Bali.</p>
                    
                    <p style={{fontSize:'1.01vw',fontFamily:'National2Light'}}>
                    <i>Ultimates</i> dapat menginap di DESA <b>PENGLIPURAN</b>, menyusuri desa yang asri dan <br />
                         dikelilingi pohon bambu sambil bersepeda, hingga, mencoba masakan khas desa.     
                    </p>
                </div>
            </div>

            <div className='penglipuranSectionKanan'>
                <div className="penglipuranSectionKanan_dekorasiTengah">
                    <img src="./images/tengok-ragam-real/Page 3/aset2 pg3.png" alt="" />
                    <img src="./images/tengok-ragam-real/Page 3/aset3 pg3.png" alt="" />
                </div>

                <div className="penglipuranSectionKanan_content">
                    <p style={{fontFamily:'National2Light',fontSize:'1.38vw'}}>DESA <b>PENGLIPURAN</b> memiliki beberapa bagian area yang dapat  <i>Ultimates</i> jelajah.</p>
                    <p style={{fontFamily:'National2Regular',fontSize:'1.833vw'}}>Mengusung konsep Tri Mandala, <span style={{fontFamily:'National2CompressedMedium'}}>DESA</span>  <b style={{letterSpacing:'1.5px'}}>PENGLIPURAN</b> <br /> memiliki tiga bagian wilayah, yaitu</p>
                    <div>
                        <p>01</p>
                        <p>02</p>
                        <p>03</p>
                    </div>
                    <div>
                        <img src="./images/tengok-ragam-real/Page 3/aset4 pg3.png" alt="" />
                        <img src="./images/tengok-ragam-real/Page 3/aset5 pg3.png" alt="" />
                        <img src="./images/tengok-ragam-real/Page 3/aset6 pg3.png" alt="" />
                    </div>

                    <div className='penglipuranSectionKanan_deskripsi'>
                        <p style={{fontFamily:'National2Light',fontSize:'1.38vw'}}>
                        Selain itu,  <i>Ultimates</i> juga  dapat melihat masyarakat DESA PENGLIPURAN melakukan ritual keagamaan, <br />
                         salah satunya Ngusaba. Saat Ngusaba, masyarakat menghaturkan sesaji buah dan jajanan Bali <br /> 
                         sebagai ucapan syukur. Ada pun ritual ini dilakukan dalam rangka menyambut Hari Raya Nyepi.  
                        </p>
                        <br />
                        <p style={{fontFamily:'National2Light',fontSize:'1.38vw'}}>
                        Lalu, ada juga <i style={{fontFamily:'National2BoldItalic'}}>Penglipuran Village Festival</i> , yaitu acara yang diadakan di akhir tahun. Pada acara ini, ada parade pakaian <br />
                         adat Bali, parade seni budaya, dan pentas Barong Ngelawang yang dapat dinikmati oleh wisatawan yang berkunjung. 
                        </p>
                    </div>
                </div>
               
            </div>
        </motion.div>
    )
}

export default Penglipuran;